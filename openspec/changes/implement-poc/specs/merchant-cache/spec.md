...
The system SHALL maintain an in-memory `HashMap` mapping normalised merchant keys to their classification result (merchant name, category, confidence). When a transaction's normalised key matches a cached entry, the cached result SHALL be returned without calling the LLM.

#### Scenario: Cache hit avoids LLM call

- **WHEN** a transaction with normalised key "SBB MOBILE" is classified and the cache already contains an entry for "SBB MOBILE"
...
- **THEN** the system SHALL call the LLM, store the result in the cache, and return it
